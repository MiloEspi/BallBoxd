...
export default function RateMatchModal({
  ...
  const minutesOptions = [
    { value: 'LT_30', label: t('match.minutes.lt30') },
    { value: 'ONE_HALF', label: t('match.minutes.oneHalf') },
    { value: 'ALMOST_ALL', label: t('match.minutes.almostAll') },
    { value: 'FULL', label: t('match.minutes.full') },
  ];
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t('rate.saveError'));
    } finally {
      setLoading(false);
    }
  };
          ...
          <form className="space-y-6 px-6 py-6" onSubmit={handleSubmit}>
            ...
            <div className="space-y-2">
              <label className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">
                {t('rate.minutesWatched')}
              </label>
              <SegmentedControl
                options={minutesOptions}
                value={minutesWatched}
                onChange={setMinutesWatched}
                ariaLabel={t('rate.minutesWatched')}
              />
            </div>
            ...
            <button
              ...
              disabled={loading}
            >
              {loading ? t('rate.saving') : t('rate.submit')}
            </button>
          </form>
        </div>
      </div>
